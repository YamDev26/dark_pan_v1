      ...
      <div class="d-flex">
        <div class="mx-3">
          <select class="form-select form-select w-auto border-0 text-color-3" onchange="window.location.href=this.value;" {{ $evaluat['actif'] != 1 ? 'disabled':'' }}>
            ...
            @if (!$existe)
              <option value="{{ route('evaluation.create', $evaluat['id']) }}">Add Note</option>
            @else
              <option value="{{ route('evaluation.editNot', $evaluat['get_classe_id'].'_'.$evaluat['id']) }}">Edit Note</option>
              <option value="#">Get Pdf</option>
            @endif
          </select>
        </div>
        <a href="{{ route('evaluation.show',($evaluat['get_classe_id'].'_'.$evaluat['level_matter_id'])) }}" class="btn btn-outline-light ml-2 py-1">Return</a>
      </div>
    </div>
...
<script>
  $(document).ready(function() {
    ...
    $('#myTable').DataTable({
      ...
      serverSide: true,
      ordering: false,
      ajax: '{{ route('evaluation.dataTables', $evaluat['get_classe_id'].'_'.$evaluat['id']) }}',
      columns: [
