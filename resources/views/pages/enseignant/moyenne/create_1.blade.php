@extends('app')
@section('title', 'Add moyenne '.$classe['libelle'])
@section('link')
  ...
  <div class="bg-secondary text-center rounded p-4">
    <div class="d-flex align-items-center justify-content-between mb-4 pb-2" style="border-bottom: 1px solid #6C7293">
      ...
      <Div class="d-flex">
        ...
        <a href="{{ route('moyennes.show', ($classe->id.'_'.$matter->id.'_'.$cutting->id)) }}" class="btn btn-outline-light py-1">Return</a>
      </Div>
    </div>
    <form action="{{ route('moyennes.store', ($classe->id.'_'.$matter->id.'_'.$cutting->id)) }}" method="post" id="myForm">
      @csrf
...
@include('partials._modal_import',[
  'url' => route('moyennes.import', ($classe->id.'_'.$matter->id.'_'.$cutting->id)),
  'export' => route('moyennes.export', ($classe->id.'_'.$matter->id.'_'.$cutting->id))
])
@endsection
@section('script')
