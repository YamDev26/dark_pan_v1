      ...
      <div class="bg-secondary rounded h-100 p-4">
        <div class="mb-4 d-flex justify-content-between" style="border-bottom: 1px solid #6C7293">
          ...
          <div class="d-flex">
            ...
            <a href="{{ route('teacher.index') }}" class="btn btn-outline-light m-2 py-1">Return</a>
          </div>
        </div>
        <form action="{{ route($data ? 'teacher.update':'teacher.store', ($data ? $data['id']:'')) }}" method="POST" id="myForm">
            ...
            <div class="row px-sm-3">
              <div class="col-4 mb-2">
                ...
                <div class="form-check form-check-inline ml-2">
                  <input type="radio" name="enseignant" class="form-check-input @error('enseignant') is-invalid @enderror" id="permanent" value="permanent" {{ $data ? ($data->teacher->type == 'permanent' ? 'checked':''): (old('enseignant') == 'permanent' ? 'checked':'checked') }}>
                  <label class="form-check-label" for="permanent">Permanent</label>
                </div>
                <div class="form-check form-check-inline">
                  <input type="radio" name="enseignant" class="form-check-input @error('enseignant') is-invalid @enderror" id="vacataire" value="vacataire" {{ $data ? ($data->teacher->type == 'vacataire' ? 'checked':''): (old('enseignant') == 'vacataire' ? 'checked':'') }}>
              ...
              <div class="col-4 mb-2">
                ...
                <div class="form-check form-check-inline ml-2">
                  <input type="radio" name="autorisate" class="form-check-input @error('autorisate') is-invalid @enderror" id="non" value="non" {{ $data ? (!$data->teacher->autorisate ? 'checked':''): (old('autorisate') == 'non' ? 'checked':'') }}>
                  <label class="form-check-label" for="non">Non</label>
                </div>
                <div class="form-check form-check-inline">
                  <input type="radio" name="autorisate" class="form-check-input @error('autorisate') is-invalid @enderror" id="oui" value="oui" {{ $data ? ($data->teacher->autorisate ? 'checked':''): (old('autorisate') == 'oui' ? 'checked':'checked') }}>
            ...
            <div class="row px-sm-3">
              ...
              <div class="col-12 col-sm-6 mb-2">
                ...
                <select name="etude" class="form-select @error('etude') is-invalid @enderror" id="etude" aria-label="Default select example">
                  <option value="">Select ...</option>
                  <option value="1" {{ old('etude') == '1' ? 'selected':'' }} {{ $data ? ($data->teacher->etude == '1' ? 'selected':''):'' }}>Bac+1</option>
                  <option value="2" {{ old('etude') == '2' ? 'selected':'' }} {{ $data ? ($data->teacher->etude == '2' ? 'selected':''):'' }}>Bac+2</option>
                  <option value="3" {{ old('etude') == '3' ? 'selected':'' }} {{ $data ? ($data->teacher->etude == '3' ? 'selected':''):'' }}>Bac+3</option>
                  <option value="4" {{ old('etude') == '4' ? 'selected':'' }} {{ $data ? ($data->teacher->etude == '4' ? 'selected':''):'' }}>Bac+4</option>
                  <option value="5" {{ old('etude') == '5' ? 'selected':'' }} {{ $data ? ($data->teacher->etude == '5' ? 'selected':''):'' }}>Bac+5</option>
                </select>
              </div>
              <div class="col-12 col-sm-6 mb-2">
                <label for="diplom" class="col-form-label">Dernier diplôme<span class="text-danger">*</span> :</label>
                <input type="text" name="diplom" class="form-control @error('diplom') is-invalid @enderror" id="diplom" value="{{ old('diplom', $data ? $data->teacher->diplome:null) }}" placeholder="Dernier Diplôme Enseignant">
              </div>
              <div class="my-0" id="divAuto" style="display: none">
                <div class="row my-0">
                  <div class="col-12 col-sm-6 mb-2">
                    <label for="num_auto" class="col-form-label">Numéro autorisation<span class="text-danger">*</span> :</label>
                    <input type="text" name="num_auto" class="form-control @error('num_auto') is-invalid @enderror" id="num_auto" value="{{ old('num_auto', $data ? $data->teacher->num_autorisate:null) }}" placeholder="Numéro Autorisation">
                  </div>
                  <div class="col-12 col-sm-6 mb-2">
                    <label for="date_acquise" class="col-form-label">Date d'acquisition :</label>
                    <input type="date" name="date_acquise" class="form-control @error('date_acquise') is-invalid @enderror" id="date_acquise" value="{{ old('date_acquise', $data ? $data->teacher->date_autorisate:null) }}">
                  </div>
                </div>
              </div>
              <div class="col-12 col-sm-6 mb-2">
                ...
                <select name="matter" class="form-select @error('matter') is-invalid @enderror" id="matter">
                  ...
                  @foreach ($matters as $item)
                    <option value="{{ $item['id'] }}" {{ old('matter') == $item['id'] ? 'selected':'' }} {{ $data ? ($data->teacher->matter_id == $item['id'] ? 'selected':''):'' }}>{{ ucwords($item['symbol']) }}</option>
                  @endforeach
                </select>
              </div>
              <div class="col-12 col-sm-6 mb-2">
                <label for="experiens" class="col-form-label">Expérience professionnelle<span class="text-danger">*</span> :</label>
                <input type="text" name="experiens" class="form-control @error('experiens') is-invalid @enderror" id="experiens" value="{{ old('experiens', $data ? $data->teacher->experiens:null) }}" placeholder="Expérience Professionnelle Enseignant">
              </div>
