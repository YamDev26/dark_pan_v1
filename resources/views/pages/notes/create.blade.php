...
<div class="container-fluid pt-4 px-4">
  <div class="bg-secondary text-center rounded p-4">
    ...
    <form action="{{ route('note.store', $evaluat['id']) }}" method="post" id="myForm">
      @csrf
        ...
        <table class="table table-bordered table-hover align-middle" id="myTable">
          <thead>
            <tr class="text-white">
              ...
              <th scope="col" class="text-center" style="width: 14%">Notes</th>
            </tr>
          </thead>
          <tbody>
            @foreach ($datas as $i => $item)
              <tr>
                <th class="text-center">{{ $i < 9 ? '0'.($i+1):($i+1) }}</th>
                <th class="text-left">{{ $item->matricul }}</th>
                <th class="text-left">{{ strtoupper($item->first) }}</th>
                <th class="text-left">{{ ucwords($item->last) }}</th>
                <th class="text-left">{{ $item->genre == 'F' ? 'Feminin':'Masculin' }}</th>
                <th class="d-flex py-0 text-center">
