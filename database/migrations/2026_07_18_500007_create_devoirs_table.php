...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('devoirs', function (Blueprint $table) {
            ...
            $table->foreign('devoirs_type_id')->references('id')->on('devoirs_types')->onDelete('cascade');
            $table->foreign('cutting_school_year_id')->references('id')->on('cutting_school_years')->onDelete('cascade');
            $table->unique(['get_classe_id', 'dates', 'debut', 'cutting_school_year_id'], 'gddc_unique'); // Contrainte unique sur les deux colonnes
            $table->timestamps();
        });
    }
